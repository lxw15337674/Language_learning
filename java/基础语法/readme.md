...
####枚举类
	一些方法在运行时,它需要的数据不能是任意的,而必须是一定范围内的值,可以直接使用枚举
	...
	注意细节:
		...
		3.枚举值是枚举值所属的类的数据类型,然后枚举值是指向了本类的对象的.
		4.枚举类的构造方法默认的修饰符是private的.
		5.枚举类可以定义自己的成员变量与成员函数.
		6.枚举类可以自定义构造函数,但是构造函数的修饰符必须是private
		7.抽象类可以存在抽象的方法,但是枚举值必须实现抽象的方法.
		8.抽象值必须要位于枚举类的第一个语句.
		9.如果使用switch时,case语句后面跟的枚举值,只需要但写枚举值就行,不用再声明枚举类.
##IO流(input output):
	IO技术只要的作用是解决设备与设备之间的数据传输问题.  比如:硬盘-->内存  内存的数据-->硬盘上
	IO流技术:
	...
	IO流分类:
		如果按照数据的流向划分:
			...
			输出流
		如果按照处理的单位划分:
			字节流:	字节流读取文件中的二进制数据,读取到二进制数据不会经过任何的处理.
			字符流:	字符流读取的数据是以字符为单位的.字符流也是读取文件中的二进制数据,不过会把这些数据转换成我们能识别的字符.
				字符流=字节流+解码

###字节流和字符流
	字节流:字节流读取的是文件中的二进制数据,读到的数据并不会帮你转换成看得懂的字符.
	字符流:字符流会把读取到的二进制的数据进行对应的编码与解码工作. 字符流=字节流+转码(解码)
		使用字符流的应用场景:
			读取字符数据的时候则使用字符流. 例如:文字
		使用字节流的应用场景:
			读写的数据都不需要转换成字符的时候,则使用字节流.  例如:图片
####Buffered缓冲方法
	readLine() 读取一行文本
####输入字节流
	...
	----------|	FileInputStream	读取文件数据的输入字节流
					使用FileInputStream读取文件数据的步骤:
						...
						3.读取文件中的数据.
						4.关闭资源.(先开后关)
	----------| BufferedInputStream 缓冲输入字节流 作用:为了提高读取文件数据的效率.
					该类内部只不过是维护了一个8kb的字节数组. 
					注意:BufferedInputStream本身不具备读文件能力,所以需要借助FileInputStream来读取文件的数据
					步骤:
						1.找到目标文件
						2.建立数据的输入通道.
						3.建立缓冲输入字节流
...
####输出字节流	
	...
	----------| FileOutStream 向文件输出数据的输出字节流
					步骤:
						...
						4.关闭资源.(先开后关)
					注意细节:
						1.使用FileOutPutStream时,如果目标文件不存在,就会自动创建目标文件对象.
						2.使用FileOutPutStream写数据时,如果目标文件已经存在,那么会先清空原有数据,再写入数据.
						3.使用FileOutPutStream写数据时,如果目标文件已经存在,需要在原来数据基础上追加数据的时候应该使用new FileOutStream(file,true)构造函数.
						4.使用write方法写数据的时候,虽然接收的是一个int类型的数据,但是真正写出的只是一个字节的数据,只是把
							低八位的二进制数据写出,其他二十四数据全部丢弃.
	----------| BufferedOutputStream 缓冲输入字节流
					该类内部只不过是维护了一个8kb的字节数组. 
					注意:
						1.使用BufferedOutputStream写数据时,它的write方法是先把数据写到它内部维护的字节数组中.
						2.写数据时,如果需要把数据真正的写到硬盘上面,三种方法:
							1.flush方法
							2.close方法(close方法会执行flush方法后关闭stream)
							3.内部维护的字节数组已经填满数据的时候.
...
####输出字符流
	...
	----------| FileWriter 向字符数据数据的输出字符流
				步骤:
					...
					4.关闭数据.(先开后关)
				注意:
					1.FileWriter内部是维护了一个1024个字符数组的如果需要把数据写到硬盘上需调用flush或close方法或填满内部的字符数组.
					2.使用FileWriter如果目标文件不存在,会自动创建目标文件.如果目标文件已经存在,会先清空数据,再写入数据.如果需要在原来的基础上追加数据,需要使用FileWriter(File ,boolean)
...
###File类:用于读取文件或文件夹的属性数据.
####目录分隔符:
	在windows系统上的目录分隔符是\
	在linux系统上的目录分隔符是/
	注意:
		在windows系统上\与/都可以使用作目录分隔符.而且,如果写/的时候只需要写一个即可.
####File类的构造方法
	File(String pathname) 指定路径创建一个File对象
	File(File parent,String child)	根据parent抽象路径名和child路径名字创建一个新File独享
####路径问题:
	绝对路径:该文件在硬盘上的完整路径.
...
####方法
	创建：
		...
		mkdir()			在指定位置创建目录，只能创建单级文件夹。
		mkdirs()		在指定位置创建目录，可以创建多级文件夹。
		renameTo(File dest)	重命名文件或文件夹，也可以操作非空的文件夹.如果目标文件与源文件不是在同一个路径下,那么renameTo作用就是剪切功能.
	删除:
		delete()	删除文件或一个空文件夹,如果是文件夹且不为空,这不能删除.
		deleteOnExit()	jvm退出的时候删除文件,一般用于删除临时文件.
	判断:
		exists()		文件或文件夹是否存在。
